...
def dice_roll(canvas):
    ...
    dice_roll_val = dice()
    dice_img = PhotoImage(file=r'Assets/' + dice_face[dice_roll_val])
    canvas.dice_img = dice_img  # To prevent asset from being garbage collected.
    canvas.create_image(500, 400, image=dice_img)
...
def make_board():
    ...
    canvas = Canvas(window, width=650, height=700, bg="white")
